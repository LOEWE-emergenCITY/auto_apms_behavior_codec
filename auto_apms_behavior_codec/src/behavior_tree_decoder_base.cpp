...
BehaviorTreeDecoderBase::BehaviorTreeDecoderBase(
  ...
  std::shared_ptr<auto_apms_behavior_codec::DictionaryManager> dictionary_manager)
{
  ...
  for (const auto & port : node.ports) {
    if (!port) {
      RCLCPP_WARN(
        ...
        "Null port pointer detected in node '%s'", node.type_name.c_str());
      continue;
    }
    ...
    if (port_id < dict_node.port_types.size()) {
      ...
      RCLCPP_DEBUG(
        rclcpp::get_logger("behavior_tree_decoder"),
        "Port ID %u maps to name '%s' for node '%s'",
        port_id, port_name.c_str(), node.type_name.c_str());
    } else if (node.type_name == "SubTree") {
      if (std::dynamic_pointer_cast<behavior_tree_representation::PortBool>(port)) {
        port_name = "_autoremap";
      } else if (
        auto port_subtree =
          std::dynamic_pointer_cast<behavior_tree_representation::PortSubTreeSpecial>(port))
      {
      ...
      RCLCPP_WARN(
        rclcpp::get_logger("behavior_tree_decoder"),
        "Port ID %u out of range for node '%s' (has %zu ports)",
        port_id, node.type_name.c_str(), dict_node.port_types.size());
      continue;
    }

    if (auto port_string =
      std::dynamic_pointer_cast<behavior_tree_representation::PortString>(port))
    {
      port_value = port_string->value;
    } else if (
      auto port_int = std::dynamic_pointer_cast<behavior_tree_representation::PortInt>(port))
    {
      port_value = std::to_string(port_int->value);
    } else if (
      auto port_uint = std::dynamic_pointer_cast<behavior_tree_representation::PortUInt>(port))
    {
      port_value = std::to_string(port_uint->value);
    } else if (
      auto port_float = std::dynamic_pointer_cast<behavior_tree_representation::PortFloat>(port))
    {
      port_value = std::to_string(port_float->value);
    } else if (
      auto port_double =
        std::dynamic_pointer_cast<behavior_tree_representation::PortDouble>(port))
    {
      port_value = std::to_string(port_double->value);
    } else if (
      auto port_bool = std::dynamic_pointer_cast<behavior_tree_representation::PortBool>(port))
    {
      port_value = port_bool->value ? "true" : "false";
    } else if (
      auto port_any =
        std::dynamic_pointer_cast<behavior_tree_representation::PortAnyTypeAllowed>(port))
    {
        ...
        std::dynamic_pointer_cast<behavior_tree_representation::PortSubTreeSpecial>(port))
    {
      ...
      auto port_invalid =
        std::dynamic_pointer_cast<behavior_tree_representation::PortInvalid>(port))
    {
      ...
      RCLCPP_WARN(
        rclcpp::get_logger("behavior_tree_decoder"),
        "Unknown port type in node '%s'", node.type_name.c_str());
      continue;
    }
  ...
  RCLCPP_DEBUG(
    rclcpp::get_logger("behavior_tree_decoder"),
    "Port map for node '%s' has %zu entries", node.type_name.c_str(), port_map.size());
  return port_map;
}
  ...
  std::shared_ptr<auto_apms_behavior_codec::DictionaryManager> dictionary_manager)
{
  RCLCPP_DEBUG(
    rclcpp::get_logger("behavior_tree_decoder"),
    "Constructing node '%s'", node.type_name.c_str());
  ...
  for (const auto & [port_name, port_value] : port_map) {
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("behavior_tree_decoder"),
      "Set port attribute '%s'='%s' on node '%s'",
      port_name.c_str(), port_value.c_str(), node.type_name.c_str());
  }
  ...
  for (const auto & child : node.children) {
    if (!child) {
      RCLCPP_WARN(
        rclcpp::get_logger("behavior_tree_decoder"),
        "Null child node pointer detected in parent '%s'", node.type_name.c_str());
      continue;
    }
    RCLCPP_DEBUG(
      rclcpp::get_logger("behavior_tree_decoder"),
      "Inserting child node '%s' into parent '%s'",
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("behavior_tree_decoder"),
      "Finished constructing child node '%s'", child->type_name.c_str());
  }

  RCLCPP_DEBUG(
    rclcpp::get_logger("behavior_tree_decoder"),
    "Finished constructing node '%s'", node.type_name.c_str());
  return base_node;
}

std::string BehaviorTreeDecoderBase::reconstructXML(
  const behavior_tree_representation::Document & document)
{
  ...
  for (const auto & tree : document.trees) {
    RCLCPP_DEBUG(
      this->get_logger(),
      "Processing tree '%s' for XML reconstruction", tree.name.c_str());
    ...
    RCLCPP_DEBUG(
      this->get_logger(),
      "Finished processing tree '%s'", tree.name.c_str());
    RCLCPP_DEBUG(
      this->get_logger(),
      "Current state of TreeDocument after processing tree '%s':\n%s",
      tree.name.c_str(), tree_doc.writeToString().c_str());
  }
...
BehaviorTreeDecoderBase::getTreeElementFromTree(
  const behavior_tree_representation::Tree & tree,
  auto_apms_behavior_tree::core::TreeDocument & tree_doc)
{
  RCLCPP_INFO(
    this->get_logger(),
    "Constructing TreeElement for tree '%s'", tree.name.c_str());
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Finished constructing TreeElement for tree '%s'", tree.name.c_str());
  return tree_element;
}
