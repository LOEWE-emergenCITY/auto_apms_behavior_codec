...
static bool parse_node(
  CborValue * nodeVal, std::shared_ptr<auto_apms_behavior_codec::DictionaryManager> dict_mgr, Node & out_node)
{
  ...
  std::cerr << "[CBOR DEBUG] parse_node: typeId=" << typeId << " name='" << out_node.type_name
            << "' nodeArrayLen=" << nodeArrayLen << std::endl;
  ...
  std::cerr << "[CBOR DEBUG] After advance past typeId, nodeIt type=" << cbor_value_get_type(&nodeIt)
            << " is_array=" << cbor_value_is_array(&nodeIt) << std::endl;
  ...
  for (size_t pi = 0; pi < portsCount; ++pi) {
    std::cerr << "[CBOR DEBUG] Parsing port index " << pi << ", CBOR type: " << cbor_value_get_type(&portsIt)
              << std::endl;
    ...
    if (!cbor_value_is_array(&portsIt)) {
      std::cerr << "[CBOR DEBUG] Port element is not an array at index " << pi
                << ", CBOR type: " << cbor_value_get_type(&portsIt) << std::endl;
      return false;
    }
    ...
    std::cerr << "[CBOR DEBUG] Node '" << out_node.type_name << "' port index " << pi
              << ": first elem type=" << firstElemType << " array_len=" << portArrayLen << std::endl;
    if (firstElemType != CborIntegerType) {
      ...
      std::cerr << "[CBOR DEBUG] After leave_container (invalid), next CBOR type at port index " << (pi + 1) << ": "
                << cbor_value_get_type(&portsIt) << std::endl;
      ...
      if (out_node.type_name == "SubTree") {
        if (portArrayLen == 3) {
          ...
          out_node.ports.push_back(
            std::make_shared<PortSubTreeSpecial>(attrValue, attrName, static_cast<int16_t>(portId)));
          std::cerr << "[CBOR DEBUG] Decoded SubTree port: name='" << attrName << "' value='" << attrValue
                    << "' id=" << portId << std::endl;
        } else {
        ...
        if (portType == "int") {
          ...
          out_node.ports.push_back(
            std::make_shared<PortNodeStatus>(
              PortNodeStatus::getEnumString(static_cast<BT::NodeStatus>(value)), static_cast<int16_t>(portId)));
        } else {
    ...
    std::cerr << "[CBOR DEBUG] After leave_container, next CBOR type at port index " << (pi + 1) << ": "
              << cbor_value_get_type(&portsIt) << std::endl;
